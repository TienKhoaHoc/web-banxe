...
function addThongKe() {
    var dataChart = {
        type: 'bar',
        data: {
            labels: ["Honda", "Yamaha", "Suzuki", "SYM", "Vinfast", "Khac"],
            datasets: [{
...
function refreshTableKhuyenMai(){
    $.ajax({
        ...
        error: function(e) {
            Swal.fire({
                type: "error",
                title: "Lỗi lấy dữ liệu sản phẩm (admin.js > refreshTableKhuyenMai)",
                html: e.responseText
            });
            console.log(e.responseText)
        }
...
function addTableKhuyenMai(list_KhuyenMai) {
    ...
    for (var i = 0; i < list_KhuyenMai.length; i++) {
        var p = list_KhuyenMai[i];
        s += `<tr>
            <td style="width: 5%">` + (i + 1) + `</td>
            <td style="width: 20%">` + p.MaKM + `</td>
            <td style="width: 40%">` + p.TenKM + ` </td>
            <td style="width: 15%">` + p.LoaiKM + `</td>
            <td style="width: 20%">` + p.GiaTriKM + `</td>
        </tr>`;
    }
        ...
        <table class="overlayTable table-outline table-content table-header">
            ...
            <tr>
                ...
                <td>
                    ...
                    var company = ["Honda", "Yahama", "SYM", "Winfast", "Suzuki"];
                    var i = 1;
                ...
                <?php
                    if (file_exists("img/products/" . basename($_FILES["hinhanh"]["name"]))) {
                        echo "Error Code: " . $_FILES["hinhanh"]["error"] . " (Chỉnh sửa ảnh lại sau)";
                    } else {
                        ...
                        $tenfilemoi = "img/products/" . $tenfilemoi; // Chỉnh sửa ở đây, sử dụng $tenfilemoi thay vì $_FILES["hinhanh"]["name"]
            ...
            <tr>
                <td>Phuộc Trước:</td>
                <td><input type="text" value="` + sp.PhuocTruoc + `"></td>
            </tr>
            <tr>
                <td>Phuộc Sau:</td>
                <td><input type="text" value="` + sp.PhuocSau + `"></td>
            </tr>
            <tr>
                <td>Dài x Rộng x Cao:</td>
                <td><input type="text" value="` + sp.XYZ + `"></td>
            </tr>
            <tr>
                <td>Dung Tích Nhớt Máy:</td>
                <td><input type="text" value="` + sp.NhotMay + `"></td>
            </tr>
            <tr>
                <td>Mức Tiêu Thụ Nhiên Liệu:</td>
                <td><input type="text" value="` + sp.NhienLieu + `"></td>
            </tr>
            <tr>
                <td>Moment Cực Đại:</td>
                <td><input type="text" value="` + sp.Moment + `"></td>
            </tr>
            <tr>
                <td>Dung Tích Xy-lanh:</td>
                <td><input type="text" value="` + sp.Xylanh + `"></td>
            </tr>
            <tr>
...
function refreshTableHang(){
    $.ajax({
        ...
        error: function(e) {
            Swal.fire({
                type: "error",
                title: "Lỗi lấy dữ liệu sản phẩm (admin.js > refreshTableHang)",
                html: e.responseText
            });
            console.log(e.responseText)
        }
...
function addTableHang(list_Hang) {
    var tc = document.getElementsByClassName('hang')[0].getElementsByClassName('table-content')[0];
    var s = `<table class="table-outline hideImg">`;
    ...
    for (var i = 0; i < list_Hang.length; i++) {
        ...
        s += `<tr>
            <td style="width: 5%">` + (i + 1) + `</td>
            <td style="width: 10%">` + p.MaLSP + `</td>
            <td style="width: 30%">` + p.TenLSP + ` </td>
            <td style="width: 25%">` + p.HinhAnh + `</td>
            <td style="width: 10%">` + p.Mota + `</td>
            <td style="width: 10%">
                <div class="tooltip">
...
function getListHang() {
    ...
    for (var i = 0; i < u.length; i++) {
        for (var j = 0; j < u[i].hang.length; j++) {
            var sps = '';
            for (var s of u[i].hang[j].sp) {
...
function xoaHang(trangthai, masp, tensp) {
        if (window.confirm('Bạn có chắc muốn xóa ' + tensp)) {
            // Xóa
            $.ajax({
                type: "POST",
...
function addTableDonHang(data, data1) {
    ...
    for (var i = 0; i < data.length; i++) {
        var d = data[i];
        var ct = data1.find(item => item.MaHD === d.MaHD);
        var sp = ''+ct.MaSP+' ';
        s += `<tr>
            <td style="width: 5%">` + (i + 1) + `</td>
...
function setListUser(users) {
    // Chuyển đối tượng người dùng thành dạng dữ liệu có thể gửi đi (ví dụ: JSON)
    var userData = JSON.stringify(users);

    // Thực hiện AJAX request để cập nhật dữ liệu lên server
    $.ajax({
        type: "POST",
        url: "php/xulycapnhatdonhang.php", // Thay đổi đường dẫn tới file xử lý cập nhật dữ liệu
        ...
        success: function(data, status, xhr) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                $.ajax({
                    ...
                    data: {
                        request: "getall2",
                        userId: data[i].userId,  // Supposing userId is needed to fetch orders
                    },
                    success: function(orderData, status, xhr) {
                        // Xử lý dữ liệu đơn hàng nhận được từ server
                        for (var j = 0; j < orderData.length; j++) {
                            var donhang = {
                                ngaymua: orderData[j].ngaymua,
                                tinhTrang: orderData[j].tinhTrang,
                                sp: orderData[j].sp,  // Assuming sp is an array of products in the order
                            };
                            user.donhang.push(donhang);
                        }
                    },
                    error: function(e) {
                        // Xử lý lỗi khi lấy dữ liệu đơn hàng
...
function addTableKhachHang(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        s += `<tr>
            <td style="width: 15.5%">` + (i + 1) + `</td>
            <td style="width: 23%">` + u.Ho + ' ' + u.Ten + `</td>
            <td style="width: 22.5%">` + u.Email + `</td>
            <td style="width: 29%">` + u.TaiKhoan + `</td>           
            <td >
                <div class="tooltip">
                    <label class="switch">
