...
function addChiTietToWeb(p) {
    ...
    // Đổi title
    document.title = p.TenSP + ' - Thế giới xe ';
    ...
    var s = addThongSo('Dung Tích', p.DungTich);
    s += addThongSo('Khối Lượng', p.KhoiLuong);
    s += addThongSo('Phuộc Trước', p.PhuocTruoc);
    s += addThongSo('Phuộc Sau', p.PhuocSau);
    s += addThongSo('Dài x Rộng x Cao', p.XYZ);
    s += addThongSo('Dung Tích Nhớt Máy', p.NhotMay);
    s += addThongSo('Mức Tiêu Thụ Nhiên Liệu', p.NhienLieu);
    s += addThongSo('Moment Cực Đại', p.Moment);
    s += addThongSo('Dung Tích Xy-lanh', p.Xylanh);
    info.innerHTML = s;
